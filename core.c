...
int inputInt(void)
{
	...
	do
	{
		if (character != '\n') //checking whether the user has not pressed enter key
		{
			scanf("%d%c", &value, &character);
			if (character != '\n')
			{
				clearInputBuffer(); //cleaing the input buffer if a valid input is not enterd
...
int inputIntRange(int MIN_RANGE, int MAX_RANGE)
{
	...
	do
	{
		...
		else
		{
			...
			{
				printf("ERROR! Value must be between %d and %d inclusive: ", MIN_RANGE, MAX_RANGE);
...
char inputCharOption(const char stringArray[])
{
	...
	do
	{
		...
		{
			if (matchingChar == stringArray[i]) //gurantees that the character is present in the string
			{
				quitLoop = 1;
			}
